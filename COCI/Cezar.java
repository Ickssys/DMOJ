...
public class Cezar //problem name here
{
    ...
    public static void solve() throws IOException {
        int[] cards = new int[] {4,4,4,4,4,4,4,4,4,4,4,4};
        int N = nextInt(), sum = 0;
        for(int i = 0; i < N; i++){
            int num = nextInt();
            sum += num;
        ...
        int X = 21 - sum, less = 0, greater = 0, range = 0;
        if(X <= 13){
            range = X;
            for(int ls = 0; ls < range - 2; ls++){
                less += cards[ls];
            }
            for(int gr = range; gr < cards.length; gr++){
                greater += cards[gr];
            }
        }
        if(greater > less){
